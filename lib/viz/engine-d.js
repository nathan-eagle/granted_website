...
export default class DiscoveryMapEngine {
  ...
  loadAll(grants) {
    ...
    // Stagger node additions: 2 every 350ms for natural reveal
    let idx = 0
    const addBatch = () => {
      if (this._destroyed || idx >= internalGrants.length) return
      const batchEnd = Math.min(idx + (idx < 4 ? 2 : 3), internalGrants.length)
      for (; idx < batchEnd; idx++) {
        const grant = internalGrants[idx]
        this._revealedGrants.push(grant)
        this._addSingleGrantNode(grant)
      }
      this._revealedCount = idx

      if (idx < internalGrants.length) {
        this._timers.push(setTimeout(addBatch, 350))
      } else {
        // All placed — final setup
