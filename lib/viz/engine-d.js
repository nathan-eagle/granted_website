...
const PROVIDER_DISPLAY = {
  ...
  openai_gpt4_1:             { name: 'OpenAI',        abbrev: 'GPT', angle: Math.PI * 0.0 },
  claude_haiku:              { name: 'Claude',        abbrev: 'CL',  angle: Math.PI * 0.8 },
  // Deep research only
  grok:                      { name: 'Grok',          abbrev: 'GRK', angle: Math.PI * 0.5, deep: true },
  grok_fast:                 { name: 'Grok Fast',     abbrev: 'GKF', angle: Math.PI * 0.4, deep: true },
  perplexity_deep_research:  { name: 'Perplexity R',  abbrev: 'PXR', angle: Math.PI * 1.5, deep: true },
}
...
const PROVIDER_DASH = {
  ...
  openai_gpt4_1: '6,2,1.5,2',
  claude_haiku: '3,2',
  grok: 'double',
  grok_fast: '8,3',
  perplexity_deep_research: '1.5,2.5',
}
...
export default class DiscoveryMapEngine {
  ...
  loadAll(grants) {
    ...
    // Preserve caller's order (reranked by GPT-4.1-mini) — do not re-sort by fit
  ...
  _buildSourceBar() {
    ...
    // Show standard providers; deep-only providers appear on-demand when they fire
  ...
  _activateSource(provider) {
    const p = this._prefix
    // Dynamically add card for deep providers that weren't rendered initially
    if (!this._sourceCards[provider]) {
      const info = PROVIDER_DISPLAY[provider] || { name: provider, abbrev: provider.slice(0, 2).toUpperCase() }
      const card = document.createElement('div')
      card.className = `${p}source-card`
      card.innerHTML = `
        <div class="${p}source-icon">${info.abbrev}</div>
      ...
      `
      if (this._els.sourceBar) this._els.sourceBar.appendChild(card)
      this._sourceCards[provider] = card
      // Also add corridor line for map
      if (this._corridorGroup && info.angle != null) {
        ...
        this._corridorGroup.append('line')
          .attr('class', `corridor corridor-${provider}`)
          .attr('x1', cx).attr('y1', cy)
          .attr('x2', cx + Math.cos(info.angle) * maxLen)
          .attr('y2', cy + Math.sin(info.angle) * maxLen)
  ...
  _drawCorridors() {
    ...
    Object.keys(PROVIDER_DISPLAY).forEach(prov => {
      const info = PROVIDER_DISPLAY[prov]
      if (info.deep) return // Deep providers rendered on-demand when they fire
  ...
  _emitSonarPing(provider) {
    ...
    const typeMap = {
      ...
      openai_gpt4_1: 'Foundation', claude_haiku: 'Foundation',
      grok: 'Corporate', grok_fast: 'Corporate', perplexity_deep_research: 'Foundation',
    }
