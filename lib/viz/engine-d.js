...
.${p}detail-panel.${p}open { transform: translateX(0); }
...
export default class DiscoveryMapEngine {
  ...
  loadAll(grants) {
    ...
    this._revealedGrants = internalGrants
    this._revealedCount = internalGrants.length
    this._rerankDone = true

    // Add all nodes at once, positioned radially by fit score
    internalGrants.forEach((grant, i) => {
      const r = sizeForAmount(grant.amount)
      const maxR = Math.min(w, h) * 0.38
      const minR = 80
      const radialDist = maxR - (grant.fit / 100) * (maxR - minR)
      const angle = (i / internalGrants.length) * Math.PI * 2 + Math.random() * 0.3
      const nodeData = {
        id: grant.id,
        x: w / 2 + Math.cos(angle) * radialDist,
        y: h / 2 + Math.sin(angle) * radialDist,
      ...
      // Node group (no animation — instant)
      ...
      const gNode = this._nodeGroup?.append('g').attr('class', `${p}grant-node`).datum(nodeData)
        ...
        .on('click', (event, d) => { event.stopPropagation(); this._handleGrantClick(d) })

      gNode?.append('circle').attr('class', 'node-circle')
        .attr('r', r).attr('fill', TYPE_COLORS[grant.type])
      ...
      gNode?.append('circle').attr('class', 'fit-ring')
        .attr('r', r + 1).attr('fill', 'none')
        .attr('stroke', ringProps.color).attr('stroke-width', ringProps.width)
        .attr('opacity', 0.8)
        .attr('stroke-dasharray', providerDash !== 'none' ? providerDash : null)

      // Amount label for big grants
      if (grant.amount > 200000) {
        gNode?.append('text').attr('class', 'amount-label')
          .attr('text-anchor', 'middle').attr('y', -(r + 8))
          .attr('font-family', 'DM Sans, sans-serif').attr('font-size', '10px')
          .attr('font-weight', '600').attr('fill', '#B8940E').attr('opacity', 0.8)
      ...
      gNode?.append('circle').attr('class', 'fit-arc')
    ...
    // Name + fit labels
    this._labelGroup?.selectAll(`.${p}label-name`).data(this._activeNodes.filter(n => !n.isOrg), d => d.id)
      .enter().append('text').attr('class', `${p}label-name`)
      ...
      .text(d => d.grant?.name.slice(0, 25) + (d.grant?.name.length > 25 ? '...' : ''))

    this._labelGroup?.selectAll(`.${p}label-fit`).data(this._activeNodes.filter(n => !n.isOrg), d => d.id)
      .enter().append('text').attr('class', `${p}label-fit`)
      .attr('text-anchor', 'middle').attr('font-family', 'DM Sans, sans-serif')
      .attr('font-size', '8px').attr('fill', 'var(--text-secondary)').attr('opacity', 0.6)
      .attr('x', d => d.x).attr('y', d => d.y + d.radius + 25)
      .text(d => d.grant ? `${d.grant.fit}% fit` : '')
    ...
    // Render ranked list with all grants
    const rankedOrder = internalGrants.map(g => g.id)
    this._renderRankedList(rankedOrder)
    ...
    // Set ticker to summary
    this._setTicker(`${internalGrants.length} grants mapped to your organization`)
  }
  ...
  _handleGrantClick(d) {
    ...
    // --- Open panel & shift layout ---
    this._els.detailPanel?.classList.add(`${p}open`)
    this._els.canvas?.classList.add(`${p}detail-open`)
    this._els.legend?.classList.add(`${p}detail-open`)
    this._els.labor?.classList.add(`${p}detail-open`)
  ...
  _closeDetail() {
    ...
    // Close panel & restore layout
    this._els.detailPanel?.classList.remove(`${p}open`)
    this._els.canvas?.classList.remove(`${p}detail-open`)
    this._els.legend?.classList.remove(`${p}detail-open`)
    this._els.labor?.classList.remove(`${p}detail-open`)
