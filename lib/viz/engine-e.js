...
.${p}sound-btn {
  ...
  border: 1px solid rgba(181,148,14,0.4);
  color: #B8940E;
  font-family: var(--font-body);
...
.${p}main-area {
  position: absolute;
  top: 137px; left: 280px; right: 0; bottom: 0;
  overflow-y: auto;
  overflow-x: hidden;
  z-index: 1;
...
export default class RisingStakesEngine {
  ...
  init(container, config) {
    ...
    this.config = config || {}
    this.soundEnabled = !!(config && config.soundEnabled)
    this._injectStyles()
    this._buildDOM()
  ...
  loadAll(grants) {
    ...
    // Create all cards and add to grid
    ...
    // Ticker
    this._updateTicker(`${this.grants.length} grants mapped to your organization`)
  ...
  _buildDOM() {
    ...
    header.innerHTML = `
      ...
      <div class="${p}header-right">
        <button class="${p}sound-btn" data-ref="soundBtn">&#128264; OFF</button>
      </div>
    `
    ...
    detail.innerHTML = `
      ...
      <div class="${p}detail-actions">
        <button class="${p}detail-btn-primary">Apply Now</button>
        <button class="${p}detail-btn-secondary">Find RFP</button>
      </div>
    `
    ...
    this.soundBtnEl = root.querySelector('[data-ref="soundBtn"]')
    ...
    if (this.soundBtnEl) {
      this.soundBtnEl.addEventListener('click', () => {
        ...
        if (this.soundEnabled) this._initAudio()
        this.soundBtnEl.innerHTML = this.soundEnabled ? '&#128266; ON' : '&#128264; OFF'
      })
    }
    ...
    // Apply Now button -> trigger onSelectCallback
    const applyBtn = detail.querySelector('.' + p + 'detail-btn-primary')
    if (applyBtn) {
      applyBtn.addEventListener('click', () => {
        if (this.selectedGrantId && this.onSelectCallback) {
          this.onSelectCallback(this.grantMap.get(this.selectedGrantId))
        }
      })
    }
  }
  ...
  _playTone(freq, dur, type = 'sine') {
    ...
    } catch (_) { /* audio errors are non-fatal */ }
  }

  _playChime(fitScore) {
    if (fitScore >= 90) {
      // Full chime for high fit
      this._playTone(523.25, 0.15)
      setTimeout(() => this._playTone(659.25, 0.2), 80)
    } else if (fitScore >= 70) {
  ...
  _playArpeggio() {
    this._playTone(261.63, 0.15)
    setTimeout(() => this._playTone(329.63, 0.15), 100)
    setTimeout(() => this._playTone(392.00, 0.25), 200)
  }

  _playMajorChord() {
    this._playTone(261.63, 0.8)
    this._playTone(329.63, 0.8)
    this._playTone(392.00, 0.8)
  }
  ...
  _createCard(grant, sizeClass) {
    ...
    const amountStr = formatAmount(grant.amount || 0)
    ...
    // Name
    html += `<div class="${p}cell-name" data-name="${grant.id}"></div>`
    ...
    html += `<span class="${p}cell-amount" data-amount="${grant.id}"></span>`
    html += `<span class="${p}cell-deadline">${grant.deadline || ''}</span>`
    html += `</div>`

    // Fit label + bar
    html += `<div class="${p}cell-fit-label" style="color:${fitColor}" data-fitlabel="${grant.id}"></div>`
    html += `<div class="${p}cell-fit-bar"><div class="${p}cell-fit-fill" data-fitfill="${grant.id}" style="background:${fitColor}"></div></div>`
  ...
  _revealCard(grant, providerKey) {
    ...
    // Sound
    this._playChime(grant.fit_score || 0)
