...
const PROVIDER_DISPLAY = {
  ...
  openai_gpt4_1:             { name: 'OpenAI',        abbrev: 'GPT' },
  claude_haiku:              { name: 'Claude',        abbrev: 'CL'  },
  // Deep research only
  grok:                      { name: 'Grok',          abbrev: 'GRK', deep: true },
  grok_fast:                 { name: 'Grok Fast',     abbrev: 'GKF', deep: true },
  perplexity_deep_research:  { name: 'Perplexity R',  abbrev: 'PXR', deep: true },
}
...
export default class RisingStakesEngine {
  constructor() {
    ...
    this.grantMap = new Map()  // id -> grant (only for displayed/loaded grants)
    this._queuedIds = new Set() // dedup tracker for addBatch queuing
    this.container = null
    this.config = null
  ...
  addBatch(provider, grants, isInitial) {
    ...
    sorted.forEach((grant, i) => {
      ...
      if (!this.grantMap.has(grant.id) && !this._queuedIds.has(grant.id)) {
        this._queuedIds.add(grant.id)  // Mark queued so next batch can't re-queue
  ...
  loadAll(grants) {
    ...
    // Preserve caller's order (reranked by GPT-4.1-mini), cap at 40
    const sorted = uniqueGrants.slice(0, 40)
    ...
    const totalAmount = this.grants.reduce((sum, g) => {
      const n = typeof g.amount === 'number' ? g.amount : parseAmountFromString(g.amount)
      return sum + n
    }, 0)
  ...
  _activateSource(providerKey) {
    ...
    // Reveal deep-only providers when they actually fire
  ...
  _openDetail(grant) {
    ...
    if (this.detailActionsEl) {
      ...
      this.detailActionsEl.innerHTML = grantUrl
        ? `<a href="${grantUrl}" rel="noopener noreferrer"
            class="${p}detail-btn-primary" style="text-decoration:none;text-align:center;display:block">View Grant</a>`
        : `<button class="${p}detail-btn-primary" disabled style="opacity:0.4;cursor:not-allowed">View Grant</button>`
    }
